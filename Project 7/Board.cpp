...
namespace cs31
{
    ...
    int Board::evaluate() const
    {
        ...
        for (int i = 1; i <= 9; i++)
        {
            for (int j = 1; j <= 9; j++)
            {
                if (mCells[i][j].getValue() != 0)  //if the cell is not a dot
                {
                    if ( mCells[i][j].isValueOK( mCells[i][j].getValue(),
                                                 buildBox(i, j), buildRow(i, j), buildColumn(i, j) ))
                        result++;
                    //add to result for each numerical Cell in the Board for which isValueOK is true
                }
            }
        }
